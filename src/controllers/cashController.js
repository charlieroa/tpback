...
exports.getCurrentSession = async (req, res) => {
    ...
    try {
        ...
        const summaryQuery = `
            WITH movements AS (
                SELECT * FROM cash_movements WHERE cash_session_id = $1
            ),
            incomes AS (
                SELECT 
                    payment_method, 
            ...
            expenses AS (
                SELECT 
                    ...
                    SUM(amount) as total
                FROM movements 
                WHERE type IN ('expense', 'payroll_advance') -- <-- ¡AQUÍ ESTÁ LA CORRECCIÓN!
                GROUP BY category
            )
            SELECT 
        ...
        const summaryRes = await db.query(summaryQuery, [sessionId]);
...
exports.closeCashSession = async (req, res) => {
    ...
    try {
        ...
        const updateQuery = `
            ...
            RETURNING *;
        `;
        const updatedResult = await db.query(updateQuery, [user_id, numericFinalAmount, expected_cash_amount, difference, session.id]);
